# Fuentes y estado de evidencia

Consultadas el 12 de septiembre de 2026. Las funciones comerciales y compatibilidades pueden cambiar; deben revalidarse antes de implementación.

## Protocolo normativo para rutinas

- [Base de evidencia y protocolo para rutinas](EVIDENCE_BASE_AND_ROUTINE_PROTOCOL.md)
- [Libros y textos de referencia para las rutinas](BOOKS_AND_TEXTBOOKS.md)

Este protocolo es obligatorio para declarar una rutina como `EVIDENCE_REVIEWED`. La biblioteca científica específica de entrenamiento, salud pública, variables de sesión, máquinas, calentamiento y cribado médico se mantiene en ese documento para conservar una relación explícita entre afirmaciones y fuentes.

## Ingeniería de software

- [ISO/IEC/IEEE 12207:2026 — ciclo de vida](https://www.iso.org/standard/90219.html)
- [ISO/IEC/IEEE 29148:2018 — requisitos](https://www.iso.org/standard/72089.html)
- [ISO/IEC 25010:2023 — calidad](https://www.iso.org/standard/78176.html)
- [SWEBOK v4.0a](https://ieeecs-media.computer.org/media/education/swebok/swebok-v4.pdf)
- [C4 Model](https://c4model.com/diagrams)
- [arc42](https://arc42.org/)

## Android

- [Guide to app architecture](https://developer.android.com/topic/architecture)
- [Offline-first](https://developer.android.com/topic/architecture/data-layer/offline-first)
- [Modularization patterns](https://developer.android.com/topic/modularization/patterns)
- [Testing strategies](https://developer.android.com/training/testing/fundamentals/strategies)
- [Macrobenchmark](https://developer.android.com/topic/performance/benchmarking/macrobenchmark-overview)
- [Baseline Profiles](https://developer.android.com/topic/performance/baselineprofiles/overview)
- [Health Connect aggregates](https://developer.android.com/health-and-fitness/health-connect/aggregate-data)
- [Health Connect write data](https://developer.android.com/health-and-fitness/health-connect/write-data)
- [ARCore supported devices](https://developers.google.com/ar/devices)

## Seguridad e IA

- [NIST SSDF 1.1](https://csrc.nist.gov/pubs/sp/800/218/final)
- [NIST AI RMF](https://www.nist.gov/itl/ai-risk-management-framework)
- [OWASP MASVS](https://mas.owasp.org/MASVS/)
- [Google MLOps](https://docs.cloud.google.com/architecture/mlops-continuous-delivery-and-automation-pipelines-in-machine-learning)
- [Google ML pipelines](https://developers.google.com/machine-learning/managing-ml-projects/pipelines)

## Productos y proyectos de referencia

- [Hevy features](https://www.hevyapp.com/features/)
- [Fitbod: cómo funciona](https://help.fitbod.me/hc/en-us/sections/360001078993-Understanding-Fitbod-How-It-Works)
- [MacroFactor: expenditure](https://help.macrofactorapp.com/en/articles/20-expenditure)
- [MacroFactor: AI food logging](https://help.macrofactorapp.com/en/articles/258-ai-food-logging)
- [Cronometer: métodos de registro](https://cronometer.com/blog/4-ways-to-log-food-on-cronometer/)
- [MyFitnessPal: registrar alimentos](https://support.myfitnesspal.com/hc/en-us/articles/360032274592-How-do-I-add-a-food-to-my-food-diary)
- [wger architecture](https://wger.readthedocs.io/en/latest/development/architecture.html)
- [Open Food Facts GitHub](https://github.com/openfoodfacts)
- [Zepp OS Side Service](https://docs.zepp.com/docs/v2/guides/framework/side-service/intro/)
- [Zepp OS Workout Extension](https://docs.zepp.com/docs/guides/workout-extension/intro/)

## Reconocimiento alimentario

- [Nutrition5k, CVPR 2021](https://openaccess.thecvf.com/content/CVPR2021/html/Thames_Nutrition5k_Towards_Automatic_Nutritional_Understanding_of_Generic_Food_CVPR_2021_paper.html)
- [Revisión de visión móvil para alimentos](https://pmc.ncbi.nlm.nih.gov/articles/PMC9818870/)
- [Evaluación comparativa de apps alimentarias](https://pmc.ncbi.nlm.nih.gov/articles/PMC11314244/)

## Hardware objetivo

- [Realme GT 6 — especificaciones México](https://www.realme.com/mx/more-products/realme-gt-6/specs)
- [Snapdragon 8s Gen 3 brief](https://docs.qualcomm.com/bundle/publicresource/87-73942-1_REV_C_Snapdragon_8s_Gen_3_Mobile_Platform_Product_Brief.pdf)

## Límites

- La arquitectura interna de productos comerciales no es pública; no se atribuyen implementaciones no verificadas.
- El inventario del gimnasio requiere levantamiento físico.
- El hardware y runtime de Tezkatli deben medirse de nuevo antes de seleccionar modelos.
- La compatibilidad Amazfit Active debe comprobarse en el dispositivo.
